...
# Modifications include applying the separating the gradient of discriminator
# loss from real examples and the gradients of discriminator loss from fake examples
# so gradient sanitization step is applied only to the 'real examples gradient'.
...
from generative_models.dp_wgan.differential_privacy.dp_sgd.per_example_gradients import per_example_gradients
...
class DPGradientDescentOptimizer(tf.train.RMSPropOptimizer):
    ...
    def __init__(self, learning_rate, eps_delta, sanitizer,
                 ...
                 batches_per_lot=1, var_list=None):
    ...
    def minimize(self, loss_pair, global_step=None, var_list=None,
                 name=None):
        ...
        Args:
          loss_pair: tuple of the loss tensors (real loss, fake loss).
        ...
        loss_real, loss_fake = loss_pair
        if self._batches_per_lot == 1:
            real_sanitized_grads = self.compute_sanitized_gradients(
                loss_real, var_list=var_list)
            fake_clipped_grads = self.compute_sanitized_gradients(
                loss_fake, var_list=var_list, add_noise=False)
            sanitized_grads = []
            for i, r_g in enumerate(real_sanitized_grads):
                sanitized_grads.append(r_g + fake_clipped_grads[i])
            sanitized_grads_and_vars = zip(sanitized_grads, var_list)
            # import pdb
            # pdb.set_trace()
