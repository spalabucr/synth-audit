...
class Match3(Mechanism):
    ...
    def __init__(self, dataset, specs, domain_info, iters=1000, weight3=1.0, warmup=False, supports=None, epsilon=None, delta=None, save=False):
        Mechanism.__init__(self, dataset, specs, domain_info)
        self.iters = iters
    ...
    def setup(self):
        ...
        elif 'age' in self.specs:
            # ADULT
            if len(self.specs) == 3:
                # tight
                self.round2 = list(combinations(list(self.domain.attrs), 2))
            else:
                self.round2 = [('relationship', 'age'), ('marital-status', 'relationship', 'age'), ('education', 'hours-per-week'), ('workclass', 'hours-per-week'), ('gender', 'hours-per-week'), ('occupation', 'education', 'hours-per-week'), ('occupation', 'workclass', 'hours-per-week'), ('occupation', 'gender', 'hours-per-week'), ('relationship', 'occupation'), ('gender', 'relationship', 'occupation')]
        elif 'Call Type' in self.specs:
            # FIRE
            self.round2 = [('ALS Unit', 'Station Area'), ('Call Type', 'ALS Unit', 'Station Area'), ('Zipcode of Incident', 'Battalion'), ('Station Area', 'Zipcode of Incident', 'Battalion')]
        else:
            self.round2 = list(combinations(list(self.domain.attrs), 2))
    ...
    def postprocess(self, n_synth):
        ...
        for i in range(self.iters // 500):
            engine.infer(self.measurements, engine='MD', callback=None)

            if i % 4 == 3 and self.save:
                self.synthetic = engine.model.synthetic_data(n_synth)
                self.synthetic = reverse_data(self.synthetic, self.supports)
                self.transform_domain()
                self.synthetic.to_csv(checkpt, index=False)
   
        if self.save and os.path.exists(checkpt):
            os.remove(checkpt)
