...
class Mechanism:
    ...
    """
    def __init__(self, dataset, specs, domain_info):
        if isinstance(dataset, str):
            self.dataset = dataset
            self.specs = json.load(open(specs, 'r'))
            domain_info = json.load(open(domain_info, 'r'))
    ...
    def load_data(self, path=None):
        ...
        if 'INCWAGE' in self.domain_info:
