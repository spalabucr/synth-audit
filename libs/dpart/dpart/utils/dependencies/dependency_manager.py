...
class DependencyManager():
    def __init__(self, epsilon: float = None, visit_order=None, prediction_matrix=None, n_bins=20, n_parents=2):
        ...
        self.n_bins = n_bins
        if isinstance(prediction_matrix, dict):
            epsilon = 0
    ...
    def infer_matrix(self, df: pd.DataFrame, fixed_root=False) -> pd.DataFrame:
        # Nodes
        if fixed_root:
            # set first column as root
            root = df.columns.tolist()[0]
        else:
    ...
    def fit(self, df: pd.DataFrame):
        ...
        elif self.prediction_matrix == "infer_fixed_root" and self.n_parents != 0:
            self.visit_order, self.prediction_matrix = self.infer_matrix(df, fixed_root=True)
        else:
            if self.prediction_matrix is not None:
                self.visit_order = list(kahn_sort(self.prediction_matrix, revert=True))
            else:
