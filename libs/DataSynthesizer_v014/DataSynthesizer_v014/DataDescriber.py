...
class DataDescriber:
    ...
    def describe_dataset_in_correlated_attribute_mode(self,
        ...
        if bayesian_network == 'infer':
            self.bayesian_network = greedy_bayes(self.df_encoded, k, epsilon / 2)
            rem_eps = epsilon - epsilon / 2
        elif bayesian_network == 'infer_fixed_root':
            self.bayesian_network = greedy_bayes(self.df_encoded, k, epsilon / 2, root_idx=0)
            rem_eps = epsilon - epsilon / 2
        else:
        ...
        self.data_description['conditional_probabilities'] = construct_noisy_conditional_distributions(
            self.bayesian_network, self.df_encoded, rem_eps)
